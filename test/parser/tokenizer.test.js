...
describe('Tokenizer', function() {
    describe('tokenize()', function() {
        ...
        describe('error handling', function() {
            it('should throw TokenizerError for symbol starting with a digit', function() {
                expect(() => tokenize('1abc', 'test.kif')).to.throw(TokenizerError);
            });

            it('should include correct file in TokenizerError', function() {
                let err;
                try { tokenize('1abc', 'myfile.kif'); } catch (e) { err = e; }
                expect(err).to.be.instanceof(TokenizerError);
                expect(err.file).to.equal('myfile.kif');
            });

            it('should include position in TokenizerError message', function() {
                let err;
                try { tokenize('1abc', 'test.kif'); } catch (e) { err = e; }
